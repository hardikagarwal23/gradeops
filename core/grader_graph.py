...
#     if state["compiled_master_transcript"] == "[EMPTY]":
#         # Keep an empty template structure so downstream main.py saves don't break
#         fallback_empty = {
#             "subject": state["rubric_data"].get("subject", "Unknown Subject"),
#             "total_max_marks": 0.0,
#             "total_marks_awarded": 0.0,
...
#     # Extract the question definitions from our state object
#     rubric_rules = state["rubric_data"].get("questions", [])
...
#     # Apply a pacing gap to protect the API request rate ceiling
#     print("⏳ Applying 15-second pacing delay for Gemini Free Tier...")
#     time.sleep(15)
...
#                 system_instruction=system_instruction,
#                 temperature=0.0,
#                 max_output_tokens=8192,  # Bumping maximum roof limit so long arrays don't truncate mid-sentence
#                 response_mime_type="application/json",
#                 response_schema=FinalGradingPayload,  # Enforces structural type-safety contract
#             )
#         )
...
#         # Check if the model wrapped the JSON block inside markdown decorations
#         if raw_text.startswith("```json"):
#             raw_text = raw_text.split("```json")[1].split("```")[0].strip()
#         elif raw_text.startswith("```"):
#             raw_text = raw_text.split("```")[1].split("```")[0].strip()
...
#     except Exception as e:
#         print(f"❌ Exception encountered during structured grading matrix synthesis: {str(e)}")
#         # Construct an automated fallback structure to prevent graph crash out
#         fallback_error = {
#             "subject": state["rubric_data"].get("subject", "Class X Examination"),
#             "total_max_marks": 80.0,
#             "total_marks_awarded": 0.0,
...
# grading_pipeline = workflow.compile()
# print("🟢 LangGraph Orchestration engine initialized with Standalone Rubric Logic!")