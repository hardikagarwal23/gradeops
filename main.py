...
# from datetime import datetime
# from dotenv import load_dotenv
# from core.database import grades_collection
# from core.grader_graph import grading_pipeline

# load_dotenv()

# async def main():
#     # 📍 BATCH CONFIGURATION PARAMS
#     target_batch = "final_2026"
#     rubric_file_path = "english_rubric.json"
    
#     # Target directory where the UI saves the uploaded folder files
#     upload_folder_dir = "./uploaded_exams/final_2026/" 
...
#     with open(rubric_file_path, "r", encoding="utf-8") as f:
#         rubric_doc = json.load(f)
#     print(f"📁 Loaded Rubric: {rubric_doc.get('subject')}")

#     # 2. SCAN THE UPLOADED FOLDER FOR ALL PDF FILES
#     if not os.path.exists(upload_folder_dir):
#         print(f"❌ Error: Upload target folder path directory not found: '{upload_folder_dir}'")
#         return
        
#     all_files = os.listdir(upload_folder_dir)
#     pdf_files = [f for f in all_files if f.lower().endswith('.pdf')]
    
#     print(f"📂 Found {len(pdf_files)} student copy documents inside the target directory.")
#     print("----------------------------------------------------")

#     # 3. THE BATCH LOOP: ITERATE THROUGH EVERY SINGLE STUDENT COPY
#     for current_pdf_file in pdf_files:
#         # Dynamically extract student_id from filename (e.g., "STUDENT_001.pdf" -> "STUDENT_001")
#         current_student_id = os.path.splitext(current_pdf_file)[0]
        
#         print(f"\n🚀 Starting Automation Chain for Student ID: '{current_student_id}'...")
#         print(f"📄 Processing file source: {current_pdf_file}")
        
#         # Initialize the LangGraph pipeline configuration for this specific student copy iteration
#         initial_graph_state = {
#             "batch_id": target_batch,
#             "student_id": current_student_id,
#             "rubric_data": rubric_doc,
...
#                 "student_id": current_student_id,
#                 "subject": eval_data.get("subject", rubric_doc.get("subject")),
#                 "total_max_marks": eval_data.get("total_max_marks", 80.0),
#                 "total_marks_awarded": eval_data.get("total_marks_awarded", 0.0),
#                 "evaluated_at": datetime.utcnow().isoformat() + "Z",
#                 "question_breakdown": eval_data.get("question_breakdown", []),
#                 "status": "COMPLETED"
